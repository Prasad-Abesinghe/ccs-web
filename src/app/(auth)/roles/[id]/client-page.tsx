...
import { ArrowLeft, Edit, Loader2, Plus, Trash, X } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { useRole, useUpdateRole, useDeleteRole, AVAILABLE_ACTIONS, NODE_LEVEL_ACTIONS, type RoleAction } from "~/hooks/use-roles";
...
const formSchema = z.object({
  ...
  actions: z.array(z.string()).min(1, { message: "Select at least one action" }),
...
export default function EditRoleClientPage({ id }: { id: string }) {
  ...
  const deleteRoleMutation = useDeleteRole(id);

  // State for level permissions
  ...
  useEffect(() => {
    if (role) {
      // Find the global role actions (node_id is null)
      const globalRoleAction = role.role_actions.find(ra => ra.node_id === null);
      const globalActions = globalRoleAction ? globalRoleAction.actions : [];
      
      // Extract node-specific permissions
      const nodeSpecificPermissions = role.role_actions
        .filter(ra => ra.node_id !== null)
        .map(ra => ({
          nodeId: ra.node_id!,
          actions: ra.actions
        }));
      
      setNodePermissions(nodeSpecificPermissions);
      
      form.reset({
        name: role.name,
        description: role.description,
        actions: globalActions,
        hasRootAccess: role.has_root_access,
        smsEnabled: role.sms_enabled,
        emailEnabled: role.email_enabled,
        msTeamsEnabled: role.ms_teams_enabled
      });
    }
  }, [role, form]);
                ...
                >
                  ...
                  <div>
                    ...
                    <FormDescription className="mb-4">
                      Select the actions this role can perform globally (across all levels)
                    </FormDescription>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      the role and may affect users who have this role assigned.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
